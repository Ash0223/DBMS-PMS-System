...
<html lang="en">
  ...
  <h3>The placed orders are as mentioned below</h3>
  <table>
    <tr>
      <th>Id</th>
      <th>UId</th>
    ...
    <?php
        $sql = "SELECT id,uid,ptype,quantity,trans_no,amount from orders";
        $result = $conn->query($sql);
        if ($result-> num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
        echo "<tr><td>". $row["id"] ."</td><td>". $row["uid"]."</td><td>". $row["ptype"] . "</td><td>". $row["quantity"] . "</td>
        <td>". $row["trans_no"] . "</td><td>". $row["amount"] . "</td></tr>";
        }
        echo "</table>";
        }
        else {
